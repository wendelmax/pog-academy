...
export default function ExameClient({ level }: { level: CertificationLevel }) {
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white p-4 sm:p-8 dark:border-neutral-800 dark:bg-neutral-900/50">
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:gap-4">
          <button
            ...
            className="w-full rounded-lg bg-neutral-900 px-6 py-3 text-sm font-medium text-white hover:bg-neutral-700 disabled:opacity-50 dark:bg-orange-600 dark:hover:bg-orange-500 sm:w-auto sm:text-base"
          >
  ...
  if (step === "alreadyCertified" && alreadyCertifiedCertificate) {
    return (
      <div className="rounded-2xl border border-orange-200 bg-orange-50 p-4 dark:border-orange-800 dark:bg-orange-950/30 sm:p-8">
        <h2 className="text-xl font-bold text-orange-900 dark:text-orange-100 sm:text-2xl">
          Você já possui esta certificação
        </h2>
        <p className="mt-2 text-sm text-orange-800 dark:text-orange-200 sm:text-base">
          O e-mail informado já está associado a um certificado aprovado para este nível.
        </p>
        <p className="mt-3 font-mono text-xs text-orange-700 dark:text-orange-300 sm:text-sm">
          {alreadyCertifiedCertificate.participantName} – {alreadyCertifiedCertificate.levelName}
        </p>
        <p className="mt-1 break-all text-xs text-orange-600 dark:text-orange-400 sm:text-sm">
          ID: {alreadyCertifiedCertificate.certificateId}
        </p>
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          <Link
            href={`/certificado/${alreadyCertifiedCertificate.certificateId}`}
            className="w-full rounded-lg bg-orange-600 px-4 py-2 text-center text-sm font-medium text-white hover:bg-orange-500 dark:bg-orange-500 dark:hover:bg-orange-400 sm:w-auto"
          >
            Ver certificado e baixar PDF
          </Link>
          <Link
            href="/certificacao"
            className="w-full rounded-lg border border-orange-600 bg-orange-50 px-4 py-2 text-center text-sm font-medium text-orange-700 dark:border-orange-500 dark:bg-orange-900/30 dark:text-orange-300 sm:w-auto"
          >
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="mt-2 text-sm text-neutral-500 dark:text-neutral-500">
          (Não é POG: a gente prepara as questões com carinho.)
        </p>
      </div>
    );
  }
  ...
  if (submitted && result) {
    return (
      <div className="rounded-2xl border border-neutral-200 bg-white p-4 dark:border-neutral-800 dark:bg-neutral-900/50 sm:p-8">
        <h2 className="text-xl font-bold text-neutral-900 dark:text-white sm:text-2xl">
          {result.passed ? "Aprovado" : "Reprovado"}
        </h2>
        <p className="mt-2 text-sm text-neutral-600 dark:text-neutral-400 sm:text-base">
          {result.passed
            ? "Parabéns. Você provou que consegue distinguir POG de práticas saudáveis (e que leu as questões)."
            : "Não foi dessa vez. O lado bom: você sabe o que NÃO fazer. Revise o material e tente de novo."}
        </p>
        <p className="mt-3 text-xs text-neutral-500 dark:text-neutral-500 sm:text-sm">
          Pontuação: {result.score}% ({result.score}/{result.totalQuestions}{" "}
          questões). Nota mínima: {result.minScore}%.
        </p>
        {result.passed && result.certificate && (
          <div className="mt-6 rounded-xl border border-orange-200 bg-orange-50 p-4 dark:border-orange-800 dark:bg-orange-950/30 sm:p-6">
            <p className="font-semibold text-orange-900 dark:text-orange-100">
              Certificado emitido
            </p>
            <p className="mt-2 break-all font-mono text-xs text-orange-800 dark:text-orange-200 sm:text-sm">
              ID: {result.certificate.certificateId}
            </p>
            <p className="mt-1 text-xs text-orange-700 dark:text-orange-300 sm:text-sm">
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
              ...
              <a
                ...
                download={`POG-Academy-${result.certificate.certificateId}.pdf`}
                className="w-full rounded-lg border border-orange-600 bg-orange-50 px-4 py-2 text-center text-sm font-medium text-orange-700 dark:border-orange-500 dark:bg-orange-900/30 dark:text-orange-300 sm:w-auto"
              >
                Baixar PDF
              </a>
              <Link
                href="/validar"
                className="w-full text-center text-sm font-medium text-orange-600 hover:underline dark:text-orange-400 sm:w-auto"
              >
                Validar certificado
              </Link>
            </div>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:gap-4">
          <Link
            href="/certificacao"
            className="w-full rounded-lg bg-neutral-900 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-neutral-700 dark:bg-orange-600 dark:hover:bg-orange-500 sm:w-auto"
          >
            Voltar à certificação
          </Link>
          <Link
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        >
          ...
          {remainingSeconds <= 300 && remainingSeconds > 0 && (
            <span className="font-sans text-sm text-amber-600 dark:text-amber-400">
              (respira)
            </span>
          )}
        </div>
      ...
      <div className="h-2 w-full overflow-hidden rounded-full bg-neutral-200 dark:bg-neutral-700">
        <div
          className="h-full rounded-full bg-orange-500 transition-all duration-300 ease-out dark:bg-orange-400"
        ...
        {exam.questions.map((q, idx) => (
          ...
          >
            ...
            <ul className="mt-4 space-y-2">
              {q.options.map((opt, i) => (
                <li key={i}>
                  <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-neutral-200 p-3 transition-colors hover:border-neutral-300 hover:bg-neutral-50 dark:border-neutral-700 dark:hover:border-neutral-600 dark:hover:bg-neutral-800/50">
                    <input
                      type="radio"
                      name={q.id}
      ...
      <div className="flex flex-wrap items-center gap-4">
        <button
          ...
          className="rounded-lg bg-neutral-900 px-6 py-3 font-medium text-white hover:bg-neutral-700 disabled:opacity-50 dark:bg-orange-600 dark:hover:bg-orange-500"
        >
